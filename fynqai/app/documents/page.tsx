...
export default function DocumentsPage() {
    ...
    return (
        <PageWrapper>
            ...
            >
                {/* ── Page Header ── */}
                <motion.div variants={staggerItem} className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-fynq-steel">
                    <div className="space-y-2">
                        <h1 className="font-ui text-4xl font-bold text-fynq-chalk tracking-tight">Governance Documents</h1>
                        <p className="font-ui text-fynq-fog text-[15px] max-w-[500px] leading-relaxed">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-6 py-2.5 bg-fynq-red/10 border border-fynq-pure rounded-xl text-fynq-pure font-ui text-[13px] font-bold uppercase tracking-widest shadow-xl shadow-fynq-red/10 transition-all hover:scale-105 active:scale-95"
                        >
                ...
                {/* ── Controls Row ── */}
                <motion.div variants={staggerItem} className="flex items-center justify-between gap-4 py-2">
                    ...
                    <div className="flex items-center glass-panel rounded-xl p-1 border-fynq-steel">
                ...
                {/* ── Documents Table ── */}
                <motion.div variants={staggerItem} className="card border-fynq-steel overflow-hidden shadow-2xl">
                    <table className="w-full font-ui text-[13.5px]">
                        <thead>
                            <tr className="bg-fynq-obsidian/5 text-fynq-fog border-b border-fynq-steel text-[11px] font-bold uppercase tracking-[0.1em]">
                                <th className="text-left px-8 py-5">Filename</th>
                                <th className="text-left px-8 py-5">Status</th>
                                <th className="text-left px-8 py-5">Size</th>
                                <th className="text-left px-8 py-5">Units</th>
                                <th className="text-right px-8 py-5">Sync Date</th>
                                ...
                                <tr key={doc.id} className="group hover:bg-fynq-obsidian/10 transition-colors">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-fynq-graphite border border-fynq-steel flex items-center justify-center text-fynq-red group-hover:scale-110 transition-transform duration-300">
                                                <FileText className="w-5 h-5" strokeWidth={1.5} />
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="text-fynq-pure font-semibold tracking-tight group-hover:text-fynq-red transition-colors duration-300">{doc.name}</span>
                                    ...
                                    <td className="px-8 py-5 text-fynq-fog">{doc.file_type}</td>
                                    <td className="px-8 py-5 text-fynq-fog">{doc.chunk_count}</td>
                                    <td className="px-8 py-5 text-fynq-fog text-right">{new Date(doc.created_at).toLocaleDateString()}</td>
                                    <td className="px-8 py-5">
