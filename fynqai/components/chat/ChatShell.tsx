...
import { Plus, MessageSquare, ShieldAlert, Send, Trash2 } from 'lucide-react';
...
export function ChatShell({ sessionId: initialSessionId, workspaceId, initialMessages }: ChatShellProps) {
        ...
        return (
            <ReactMarkdown
                ...
                components={{
                    ...
                    pre: ({ children }) => (
                        <pre className="whitespace-pre-wrap rounded-xl p-3 bg-fynq-ink border border-fynq-steel text-[13px] leading-relaxed mb-3 overflow-x-auto">
                            {children}
                        </pre>
                    ),
                    code: ({ children }) => (
                    ...
                    blockquote: ({ children }) => (
                        <blockquote className="border-l-2 border-fynq-red/60 pl-4 italic text-fynq-fog mb-3">
                            {children}
                        </blockquote>
                    ),
                }}
    ...
    return (
        <div className="flex w-full h-[calc(100vh-var(--topnav-height))] overflow-hidden bg-fynq-void relative text-fynq-chalk">
            ...
            {/* ── Inner Chat Sidebar (200px) ── */}
            <div className="hidden lg:flex w-[200px] flex-col border-r border-fynq-steel bg-fynq-ink backdrop-blur-3xl shrink-0">
                <div className="p-4 border-b border-fynq-steel">
                    <button
                        onClick={() => {
                            setCurrentSessionId(undefined);
                            setMessages([]);
                        }}
                        className="w-full h-11 flex items-center justify-center gap-2 bg-white/[0.03] border border-fynq-steel rounded-xl text-fynq-chalk font-ui text-[13px] font-bold tracking-tight hover:border-fynq-red transition-all group shadow-xl">
            ...
            <div className="flex-1 flex flex-col items-center relative overflow-hidden">
                <div className="flex-1 w-full overflow-y-auto custom-scrollbar px-6 md:px-12 pt-12">
                    <div className="max-w-[var(--chat-max)] mx-auto space-y-10 pb-32">

                        {messages.length === 0 && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center text-center justify-center py-20">
                                <div className="w-20 h-20 rounded-3xl glass-panel flex items-center justify-center mb-8 border-fynq-steel shadow-2xl relative">
                                    <span className="font-display italic text-4xl text-fynq-red font-bold">f</span>
                                    <div className="absolute inset-0 bg-fynq-red/5 blur-2xl rounded-full" />
                                </div>
                                <h2 className="font-ui text-2xl font-bold text-fynq-pure tracking-tight mb-2">Knowledge Verification Hub</h2>
                                <p className="font-ui text-fynq-fog text-[15px] max-w-[400px] leading-relaxed italic">
                        ...
                        <AnimatePresence>
                            {messages.map((message) => (
                                ...
                                >
                                    <div className="max-w-[90%]">
                                        {message.role === 'user' ? (
                                            <div className="bg-fynq-carbon border border-fynq-steel rounded-2xl rounded-tr-sm px-6 py-4 text-fynq-chalk font-ui text-[15px] leading-relaxed shadow-xl backdrop-blur-xl">
                                                {message.content}
                                            </div>
                                        ) : (
                                            <div className="flex gap-6 items-start">
                                                <div className="w-8 h-8 rounded-full bg-fynq-red flex items-center justify-center shrink-0 mt-1 shadow-fynq-red/30 shadow-[0_0_12px]">
                                                    <span className="text-white font-display italic font-bold text-[14px]">f</span>
                                                </div>
                                                <div className="space-y-4 flex-1">
                                                    {message.had_contradiction && (
                                                        <motion.div
                                                            ...
                                                            className="card-contradiction p-5 mb-2 flex items-start gap-4 animate-progress-pulse"
                                                        >
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="font-ui text-[13.5px] text-fynq-chalk leading-relaxed tracking-tight">
                                                                    Contradictory information found across your audit scope. I have merged both sources below for manual verification.
                                                                </p>
                                                            </div>
                                                        </motion.div>
                                                    )}
                                                    <div className="text-fynq-mist font-ui text-[16px] leading-relaxed py-1">
                            ...
                            {streamedText && (
                                <motion.div
                                    className="flex items-start gap-6 max-w-[90%]"
                                >
                                    <div className="w-8 h-8 rounded-full bg-fynq-red animate-pulse flex items-center justify-center shrink-0 mt-1 shadow-fynq-red/30 shadow-[0_0_12px]">
                                        <span className="text-white font-display italic font-bold text-[14px]">f</span>
                                    </div>
                                    <div className="text-fynq-mist font-ui text-[16px] leading-relaxed py-1 streaming-cursor">
                ...
                {/* ── Input Area ── */}
                <div className="w-full bg-gradient-to-t from-fynq-void via-fynq-void/90 to-transparent pt-12 pb-10 px-6 md:px-12 flex justify-center sticky bottom-0 z-20">
                    <div className="w-full max-w-[var(--chat-max)] relative">
                        <textarea
                            className="input-base w-full pr-14 py-4 resize-none h-14 min-h-[56px] max-h-32 transition-all leading-relaxed whitespace-pre-wrap overflow-hidden"
                        ...
                        <button
                            ...
                            className="absolute right-2 top-2 p-2.5 rounded-xl bg-fynq-red text-white hover:bg-fynq-crimson transition-all disabled:opacity-20 disabled:grayscale shadow-xl shadow-fynq-red/20 active:scale-95 group"
                        >
                            <Send className="w-4 h-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" strokeWidth={2.5} />
                        </button>
                        <p className="text-center mt-4 font-ui text-[11px] font-bold text-fynq-ash uppercase tracking-[0.15em] opacity-40">
