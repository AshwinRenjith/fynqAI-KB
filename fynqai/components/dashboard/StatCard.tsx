...
export function StatCard({ label, value, isRed = false, prefix = "" }: StatCardProps) {
    ...
    return (
        <motion.div
            ...
            className={`card-elevated p-8 pt-10 min-h-[160px] flex flex-col justify-end group transition-all duration-300 ${isRed && value > 0 ? 'border-fynq-red/30 bg-fynq-red/[0.02] shadow-[0_0_24px_rgba(239,68,68,0.05)]' : ''
                }`}
        >
            ...
            <p className="font-ui text-[12px] font-bold text-fynq-ash uppercase tracking-[0.15em] mb-4">
                {label}
            </p>

            <div className="flex items-baseline">
                {prefix && <span className="font-display text-4xl text-fynq-silver font-medium mr-1">{prefix}</span>}
                <motion.h2 className={`font-display text-7xl font-medium tracking-tight ${isRed && value > 0 ? 'text-fynq-red' : 'text-fynq-chalk'}`}>
                    {rounded}
                </motion.h2>
            </div>
